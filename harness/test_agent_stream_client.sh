...
echo -n "12. Verificando version 2.8.0 y versionCode 20800 (AC-BUILD-001)... "
grep -q 'version="2.8.0"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene version 2.8.0"; exit 1; }
grep -q 'android-versionCode="20800"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versionCode 20800"; exit 1; }
grep -q '"version": "2.8.0"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene version 2.8.0"; exit 1; }
echo "[OK]"
