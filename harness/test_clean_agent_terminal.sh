...
grep -q 'version="2.8.2"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versión 2.8.2"; exit 1; }
grep -q 'android-versionCode="20802"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene android-versionCode 20802"; exit 1; }
grep -q '"version": "2.8.2"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene versión 2.8.2"; exit 1; }
echo "[OK]"
...
grep -q 'version="2.8.2"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versi�n 2.8.2"; exit 1; }
grep -q 'android-versionCode="20802"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene android-versionCode 20802"; exit 1; }
grep -q '"version": "2.8.2"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene versi�n 2.8.2"; exit 1; }
echo "[OK]"
