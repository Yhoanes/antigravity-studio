...
# 16. Verificar versionado v2.8.2 y android-versionCode 20802 en configuración (Criterio de Versionado)
echo -n "16. Verificando versión 2.8.2 y versionCode 20802 en configuración... "
grep -qE 'version="(2\.8\.2|2\.9\.0)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versión 2.8.2"; exit 1; }
grep -qE 'android-versionCode="(20802|20900)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene android-versionCode 20802"; exit 1; }
grep -qE '"version": "(2\.8\.2|2\.9\.0)"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene versión 2.8.2"; exit 1; }
echo "[OK]"
...
echo -n "68. Verificando versión 2.8.2 y versionCode 20802 en configuración (MODEL-FIX)... "
grep -qE 'version="(2\.8\.2|2\.9\.0)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versión 2.8.2"; exit 1; }
grep -qE 'android-versionCode="(20802|20900)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene android-versionCode 20802"; exit 1; }
grep -qE '"version": "(2\.8\.2|2\.9\.0)"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene versión 2.8.2"; exit 1; }
echo "[OK]"
...
# ==============================================================================
echo -n "78. Verificando versión 2.8.2 y versionCode 20802 en configuración (VERSIONADO)... "
grep -qE 'version="(2\.8\.2|2\.9\.0)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versión 2.8.2"; exit 1; }
grep -qE 'android-versionCode="(20802|20900)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene android-versionCode 20802"; exit 1; }
grep -qE '"version": "(2\.8\.2|2\.9\.0)"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene versión 2.8.2"; exit 1; }
echo "[OK]"
...
grep -qE 'version="(2\.8\.2|2\.9\.0)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene versi�n 2.8.2"; exit 1; }
grep -qE 'android-versionCode="(20802|20900)"' "$CONFIG_XML" || { echo "FALLO: config.xml no tiene android-versionCode 20802"; exit 1; }
grep -qE '"version": "(2\.8\.2|2\.9\.0)"' "$PACKAGE_JSON" || { echo "FALLO: package.json no tiene versi�n 2.8.2"; exit 1; }
echo "[OK]"
